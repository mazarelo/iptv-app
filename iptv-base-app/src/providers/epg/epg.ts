...
export class EpgProvider {
  ...
  isEpdDateValid(epg) {
    ...
    const targetEnd = new Date(epg._stop).getTime();
    const targetStart = new Date(epg._start).getTime();
  ...
  convertEPGDateToReadable(data : number) {
    ...
    const formatedData = new Date(
      `${output.year}-${output.month}-${output.day} ${output.hour}:${output.min}`,
    );
    return formatedData;
  }
            ...
            handler: (data) => {
              this.getRemoteEPGList().subscribe((res) => {
                ...
                this.chooseFromRemoteEPGList(data).subscribe((response) => {
                  const url:string = 'https://mazarelo.com/iptv/epg/' + response;
  ...
  async clear() {
    const epgList = await this.storage.listAll();
    if (epgList) {
      epgList.map((el) => {
  ...
  getEPG(url: string, country) {
    return new Observable((observer) => {
      const loader = this.loadingProvider.presentLoadingDefault('Generating EPG');
      ...
      if (url.match(regex)) {
        this.downloadEPGFile(url).subscribe((res) => {
          if (res) {
            const output = this.convertXmlToJson(res);
